    ...
    return (dispatch, getState) => {
        ...
        const timeout = setTimeout(() => {
            ...
            if (isQuizFinished(state)) {
                ...
                const nextQiuz = state.quizes[state.activeQuestion + 1];
                const qiuzNext = getNextQuizValue(nextQiuz.id);
                console.log('qiuzNext:', qiuzNext)
                dispatch(quizNextQuestions((state.activeQuestion + 1), qiuzNext));
...
async function getNextQuizValue(quizId) {
    try {
        const response = await axios.get(`/quizes/${quizId}.json`);
        const quiz = response.data;
        return quiz;
